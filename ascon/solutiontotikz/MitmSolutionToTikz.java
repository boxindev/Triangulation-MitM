  ...
  public String generate() {
    ...
    for (int round = 0; round < mitmSolution.Rounds+1; round++) {
      ...
      for (int k = 0; k < 32; k++)
      {
        for (int i = 0; i < 5; i++)
        {
          ...
          if (mitmSolution.DA[round][k][i][0] == 1 & mitmSolution.DA[round][k][i][1] == 1 & mitmSolution.DA[round][k][i][2] == 1) {
            output += "\\fill[color=" + GreenColor + "] (" + (2 * k) + "," + (12 * (mitmSolution.Rounds - round) + (4 - i) + 6) + ") rectangle ++(1,1);\n";
          }
          if (mitmSolution.DA[round][k][i][0] == 0 & mitmSolution.DA[round][k][i][1] == 0 & mitmSolution.DA[round][k][i][2] == 1) {
            output += "\\fill[color=" + PinkColor + "] (" + (2 * k) + "," + (12 * (mitmSolution.Rounds - round) + (4 - i) + 6) + ") rectangle ++(1,1);\n";
          }
          if (mitmSolution.DA[round][k][i][0] == 1 & mitmSolution.DA[round][k][i][1] == 0 & mitmSolution.DA[round][k][i][2] == 1) {
      ...
      if (round > 0) {
        ...
        for (int k = 0; k < 32; k++)
          for (int i = 0; i < 5; i++)
          {
            ...
            if (mitmSolution.DC[round-1][k][i][0] == 1 & mitmSolution.DC[round-1][k][i][1] == 1){
              consumered++;
              output += "\\draw[line width=2pt, color=pink] ("+(2*k)+","+(12*(mitmSolution.Rounds-round)+(4-i)+6)+") rectangle ++(1,1);\n";
            }
             */
            if (mitmSolution.DC[round-1][k][i][1] == 1) {
              consumered++;
              linear_consumered++;
              output += "\\draw[line width=2pt, color=red] (" + (2 * k) + "," + (12 * (mitmSolution.Rounds - round) + (4 - i) + 6) + ") rectangle ++(1,1);\n";
            }
        ...
        //if (linear_consumered > 0) {
        //  output += " \\node[align=center] at ("+(2*31+3)+","+(12 * (mitmSolution.Rounds - round) + (4 - 3) + 6.4)+") {\\textbf{\\huge + " + (linear_consumered) + " LC}};\n";
        //}
        ...
        for (int k = 0; k < 32; k++)
        {
          for (int i = 0; i < 5; i++)
          {
            ...
            if (mitmSolution.DB[round][k][i][0] == 1 & mitmSolution.DB[round][k][i][1] == 1 & mitmSolution.DB[round][k][i][2] == 1)
              output += "\\fill[color="+GreenColor+"] ("+(2*k)+","+(12*(mitmSolution.Rounds-round)+(4-i))+") rectangle ++(1,1);\n";
            if (mitmSolution.DB[round][k][i][0] == 0 & mitmSolution.DB[round][k][i][1] == 0 & mitmSolution.DB[round][k][i][2] == 1)
              output += "\\fill[color="+PinkColor+"] ("+(2*k)+","+(12*(mitmSolution.Rounds-round)+(4-i))+") rectangle ++(1,1);\n";
            if (mitmSolution.DB[round][k][i][0] == 1 & mitmSolution.DB[round][k][i][1] == 0 & mitmSolution.DB[round][k][i][2] == 1)
