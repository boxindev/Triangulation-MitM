...
public class Callback extends GRBCallback {
  ...
  private MitmSolution getSolution(int solutionNumber, double obj, double[] x) {
    //try {
      ...
      int[][][][][] DC2Value  = new int[Rounds][4][4][32][2];
      ...
      for (int round = 0; round < Rounds; round++)
	    ...
	    for (int k = 0; k < 32; k++)
            for (int l = 0; l < 2; l++){
              DC2Value[round][i][j][k][l]  = (int) Math.round(x[2304*Rounds+2*(32*(3*(4*round+i)+j)+k)+l]);
      }
	  ...
	  for (int k = 0; k < 32; k++) {
	    for (int l = 0; l < 3; l++)  { 
              DPValue[round][i][k][l]  = (int) Math.round(x[3072*Rounds+3*(32*(4*round+i)+k)+l]);
	    }
      }
	  ...
	  for (int k = 0; k < 32; k++) {
	    for (int l = 0; l < 3; l++)  { 
              DP2Value[round][i][k][l]  = (int) Math.round(x[3456*Rounds+3*(32*(4*round+i)+k)+l]);
	    }
      }
	  ...
	  for (int k = 0; k < 32; k++)
          for (int l = 0; l < 2; l++){
              DC1Value[round][i][k][l]  = (int) Math.round(x[3840*Rounds+2*(32*(4*round+i)+k)+l]);
      }
	  ...
	  for (int k = 0; k < 32; k++)
          for (int l = 0; l < 2; l++){
            DC12Value[round][i][k][l]  = (int) Math.round(x[4096*Rounds+2*(32*(4*round+i)+k)+l]);

      }

      for (int i = 0; i < 4; i++) 
	for (int k = 0; k < 32; k++) {
          DomValue[i][k]  = (int) Math.round(x[4352*Rounds+32*i+k]);
      }
      for (int i = 0; i < 3; i++) {
