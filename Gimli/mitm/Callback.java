  ...
  private MitmSolution getSolution(int solutionNumber, double obj, double[] x) {
    //try {
      ...
      for (int round = 0; round < Rounds+1; round++)
	    ...
	    for (int k = 0; k < 32; k++) {
	      for (int l = 0; l < 3; l++)  { 
                DAValue[round][i][j][k][l]  = (int) Math.round(x[3*(32*(4*(3*round+i)+j)+k)+l]);
              }
      }
      ...
      for (int round = 0; round < Rounds; round++)
	    ...
	    for (int k = 0; k < 32; k++) {
	      for (int l = 0; l < 3; l++)  { 
                DBValue[round][i][j][k][l]  = (int) Math.round(x[1152*Rounds+1152+3*(32*(4*(3*round+i)+j)+k)+l]);
              }
      }
      ...
      for (int round = 0; round < Rounds; round++)
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 4; j++)
              for (int k = 0; k < 32; k++) {
                for (int l = 0; l < 3; l++)  {
                      DPValue[round][i][j][k][l]  = (int) Math.round(x[2304*Rounds+1152+3*(32*(4*(3*round+i)+j)+k)+l]);
                }
              }
      ...
      for (int round = 0; round < Rounds; round++)
          for (int i = 0; i < 3; i++)
              for (int j = 0; j < 4; j++)
                  for (int k = 0; k < 32; k++) {
                      DC1Value[round][i][j][k] = (int) Math.round(x[3456 * Rounds +1152+ 32 * (4 * (3 * round + i) + j) + k]);
                  }

      for (int i = 0; i < 4; i++) 
        for (int k = 0; k < 32; k++) {
              DomValue[i][k]  = (int) Math.round(x[3840*Rounds+1152+32*i+k]);
          }
      for (int i = 0; i < 3; i++) {
        objValue[i]  = (int) Math.round(x[x.length-1-(2-i)]);
      }
      
      return new MitmSolution(Rounds, (int) obj, DAValue,DBValue, DPValue, DC1Value, DomValue,objValue);
  }
}
