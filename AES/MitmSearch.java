...
public class MitmSearch implements Callable<Integer> {
  ...
  public Integer call() {
    ...
    //int i = 2;
    //int j = 3;
        System.out.println("For " + i + "-th match round, " + j+ "-th starting round"); 
        MitmSolutions = getMitmSolutions(i, j, keyRounds);
        String output = "output/result_"+Rounds+"_forward"+i+"_start"+j+"_obj"+MitmSolutions.get(0).objective+".json";
  ...
  private List<MitmSolution> getMitmSolutions(int fRounds, int Startr, int keyRounds) {
    try {
      GRBEnv env = new GRBEnv(true);
      env.set(GRB.IntParam.OutputFlag, (verbose) ? 1 : 0);
      env.start();

      Mitm mitm = new Mitm(env, Rounds, keyRounds, regime, fRounds, Startr);
      //if (verbose)
