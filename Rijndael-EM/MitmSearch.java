...
public class MitmSearch implements Callable<Integer> {
  ...
  private int Rounds;
  @Option(names = {"-regime","-k"}, defaultValue = "0", description = "Regime of the boomerang. Can be 0, 1 or 2 for 128, 192 or 256 bits key. Default is ${DEFAULT-VALUE}")
  private int regime;
  ...
  public Integer call() {
    ...
    if (regime == 0)
      ...
      keyRounds = (2*Rounds+4)/3;
    else
      keyRounds = Rounds/2+1;
    ...
    /*for (int i = 5; i < 6; i ++) 
      for (int j = 3; j <= 3; j ++) {
        for(int k = 4; k < 5; k++) {*/
          System.out.println("For " + i + "forward rounds, " + k + "-th state starting round");
          MitmSolutions = getMitmSolutions(i, j, k,keyRounds);
          String output = "output/result_" + Rounds  + "_statestart" + k + "_forward" + i + "_obj"+MitmSolutions.get(0).objective+".json";
  ...
  private List<MitmSolution> getMitmSolutions(int fRounds, int KeyStartr, int StateStartr, int keyRounds) {
    try {
      GRBEnv env = new GRBEnv(true);
      env.set(GRB.IntParam.OutputFlag, (verbose) ? 1 : 0);
      env.start();

      Mitm mitm = new Mitm(env, Rounds, keyRounds, regime, fRounds, KeyStartr, StateStartr);
      //if (verbose)
      ...
      if (MitmSolutions.get(0).objective>0) {
        System.out.println("The best solution has objective " + MitmSolutions.get(0).objective + "  " + MitmSolutions.get(0).obj[0] + "  " + MitmSolutions.get(0).obj[1] + "  " + MitmSolutions.get(0).obj[2]);
