...
from keccak384_solution import File2MitmSolution, FetchInitialState, FetchConditions, FetchConsumes
...
def KECCAK384_round(FILE_NAME="outputimm/keccak/result0.json", WITH_SYMMETRY=False, verbose=False):
    SLICES = 32 if WITH_SYMMETRY else 64
    red_relations = []
    keys = []
    Keccak = declare_ring(
        [Block('vr', SLICES * 10), Block('vb', SLICES * 10), Block('vg', SLICES * 25), Block('cu0', SLICES * 5),
    ...
    for k in range(SLICES):
        if k in gray_z[0]:
            ...
            state[10][k] = vg(10 * SLICES + 5 * k)
        elif k in blue_z[0]:
            state[0][k] = vb(5 * k)
            state[5][k] = vb(5 * SLICES + 5 * k)
            state[10][k] = vb(5 * k) + vb(5 * SLICES + 5 * k) + cu0(5 * k)
        else:
            state[0][k] = vr(5 * k)
            state[5][k] = vr(5 * SLICES + 5 * k)
            state[10][k] = vr(5 * k) + vr(5 * SLICES + 5 * k) + cu0(5 * k)

    for k in range(SLICES):
        if k in gray_z[1]:
            state[1][k] = vg(5 * k + 1)
            state[6][k] = vg(5 * SLICES + 5 * k + 1)
            state[11][k] = vg(10 * SLICES + 5 * k + 1)
        elif k in blue_z[1]:
            state[1][k] = vb(5 * k + 1)
            state[6][k] = vb(5 * SLICES + 5 * k + 1)
            state[11][k] = vb(5 * k + 1) + vb(5 * SLICES + 5 * k + 1) + cu0(5 * k + 1)
        else:
            state[1][k] = vr(5 * k + 1)
            state[6][k] = vr(5 * SLICES + 5 * k + 1)
            state[11][k] = vr(5 * k + 1) + vr(5 * SLICES + 5 * k + 1) + cu0(5 * k + 1)

    for k in range(SLICES):
        if k in gray_z[2]:
            state[2][k] = vg(5 * k + 2)
            state[7][k] = vg(5 * SLICES + 5 * k + 2)
            state[12][k] = vg(10 * SLICES + 5 * k + 2)
        elif k in blue_z[2]:
            state[2][k] = vb(5 * k + 2)
            state[7][k] = vb(5 * SLICES + 5 * k + 2)
            state[12][k] = vb(5 * k + 2) + vb(5 * SLICES + 5 * k + 2) + cu0(5 * k + 2)
        else:
            state[2][k] = vr(5 * k + 2)
            state[7][k] = vr(5 * SLICES + 5 * k + 2)
            state[12][k] = vr(5 * k + 2) + vr(5 * SLICES + 5 * k + 2) + cu0(5 * k + 2)
    ...
    f0 = open('state/KECCAK384/keccak384state.txt', 'w')
    for i0 in range(25):
    ...
    f = open('state/KECCAK384/keccak384cond.txt', 'w')
    ...
    f1 = open('state/KECCAK384/keccak384_C_consume.txt', 'w')
    for i0 in range(5):
        for j0 in range(SLICES):
            if j0 in consume1z[i0]:
                ...
                f1.write(' = ')
                f1.write(str(cu1(i0 + 5 * j0)))
                f1.write('\n')
    f1.close()

    f2 = open('state/KECCAK384/keccak384_D_consume.txt', 'w')
    # D
    ...
    # ftest = open('state/KECCAK384/keccak384_test.txt', 'w')
    # ftest.write(str(CState[1][11]))
    # ftest.write(" = CState[1][11]\n")
    # ftest.write(str(CState[3][10]))
    # ftest.write(" = CState[3][10]\n")
    # ftest.write(str(DState[2][11]))
    # ftest.write(" = DState[2][11]\n")
    # ftest.close()
    ...
    for i0 in range(len(consume1lane_D)):
        for j0 in range(SLICES):
            if j0 in consume1z_D[i0]:
                ...
                f2.write(' = ')
                f2.write(str(du1(consume1lane_D[i0] + 5 * j0)))
                f2.write('\n')
    f2.close()

    theta_threestep_D2A(state, DState)

    f3 = open('state/KECCAK384/keccak384_theta1_consume.txt', 'w')
    # consume
    ...
    for i0 in range(len(consume1lane_theta)):
        ...
        f3.write(' = ')
        f3.write(str(cu1theta(SLICES * 5 * int(tmplane / 5) + 5 * consume1z_theta[i0] + (tmplane % 5))))
        f3.write('\n')
    f3.close()
    ...
    f1 = open('state/KECCAK384/keccak384_C_consume_1.txt', 'w')
    for i0 in range(5):
        for j0 in range(SLICES):
            if j0 in consume1z[i0]:
                f1.write(str(CState[i0][j0]))
    ...
    f2 = open('state/KECCAK384/keccak384_D_consume_1.txt', 'w')
    # D
    DState = [[] for i in range(5)]
    for i in range(5):
        for j in range(SLICES):
    ...
    f3 = open('state/KECCAK384/keccak384_theta1_consume_1.txt', 'w')
    # consume
